...
// BaseHook definition for testing
abstract contract BaseHook {
    ...
    function getHookPermissions() public virtual returns (Hooks.Permissions memory);

    function beforeInitialize(address, PoolKey calldata, uint160, int24, bytes calldata)
        external
        virtual
        returns (bytes4)
    {
...
contract AuctionRepoHook is BaseHook, Ownable, ReentrancyGuard {
    ...
    function afterInitialize(PoolKey calldata key, uint160, int24, bytes calldata) external override returns (bytes4) {
        ...
        require(Currency.unwrap(poolKey.currency0) == address(0), "PoolKey already set");
